...
export function useDelayedInput(
  ...
  // HACK without this ref sometimes an outdated callback is used on fast keypresses
  const latestHandleInputRef = useRef(_handleInput);
  latestHandleInputRef.current = _handleInput;
  ...
  const queueInput = useCallback((input: DelayedInput) => {
    if (queueEmptyRef.current && !immediatelyHandledThisRenderRef.current) {
      ...
      latestHandleInputRef.current(input);
    } else {
